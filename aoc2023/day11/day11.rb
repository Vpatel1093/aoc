def day_11(file_name)
    ...
    galaxy_path_combinations.each do |galaxy_a, galaxy_b|
        sum_of_shortest_paths += (galaxy_b[0] - galaxy_a[0]).abs + (([galaxy_a[0], galaxy_b[0]].min..[galaxy_a[0], galaxy_b[0]].max).to_a & rows_to_expand).count
        sum_of_shortest_paths += (galaxy_b[1] - galaxy_a[1]).abs + (([galaxy_a[1], galaxy_b[1]].min..[galaxy_a[1], galaxy_b[1]].max).to_a & columns_to_expand).count
    end
